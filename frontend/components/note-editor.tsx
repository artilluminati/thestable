...
export function NoteEditor({ path }: { path: string }) {
    ...
    if (isError || !note)
        return <p className="text-sm text-red-400">Заметка не найдена</p>;
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                    <h1 className="truncate text-xl font-black font-climate-crisis-sans text-neutral-100">
                        {note.title}
                    </h1>
            ...
            <div>
                ...
                {!backlinks || backlinks.length === 0 ? (
                    <p className="text-sm text-neutral-500">
                        Пока никто не ссылается на эту заметку
                    </p>
                ) : (
                    <div className="flex flex-wrap gap-2">
                        {backlinks.map((b) => (
