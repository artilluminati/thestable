...
export function UrlList() {
  ...
  if (isError) return <p className="text-sm text-red-500">Не удалось загрузить ссылки</p>;
  if (!urls || urls.length === 0) {
    return <p className="text-sm text-neutral-500">Пока нет ни одной ссылки</p>;
  }
      ...
      {urls.map((url) => (
        <Card key={url.id} className="flex items-center justify-between p-4">
          <div className="min-w-0">
            ...
            <p className="truncate text-sm text-neutral-500">{url.original_url}</p>
            <p className="text-xs text-neutral-400">{url.clicks} переходов</p>
          </div>
          <button
